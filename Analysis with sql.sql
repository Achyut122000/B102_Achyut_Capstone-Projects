...
SELECT 
    albums.name, SUM(songs.length) AS total_length
FROM
    albums
        JOIN
...
VALUES ('Mafia Mundeer');

INSERT INTO albums (name, release_year, band_id)
VALUES ('International Villager', 2012, (SELECT id FROM bands WHERE name = 'Mafia Mundeer'));

#7. Delete the Band and Album you added in previous question.
DELETE FROM albums 
WHERE
...
SELECT 
    a.name AS album_name, s.name AS song_name, s.length
FROM
    albums a
        JOIN
...
SELECT 
    b.name AS band_name, COUNT(s.id) AS song_count
FROM
    bands b
        JOIN
...
#11. Create a decade column with dividing the year // 10 *10.
ALTER TABLE albums
...
#13. Find the album which released in 2008 to 2013.
SELECT 
    *
FROM
    albums
